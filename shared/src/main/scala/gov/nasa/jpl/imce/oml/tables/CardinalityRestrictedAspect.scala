...
(
  @(JSExport @field) override val uuid: taggedTypes.CardinalityRestrictedAspectUUID,
  ...
  def this(
    ...
    restrictionKind: CardinalityRestrictionKind)
  = this(
      taggedTypes.cardinalityRestrictedAspectUUID(oug.namespaceUUID(
        tboxUUID,
        "name" -> name).toString),
      tboxUUID,
      name,
...
@JSExportTopLevel("CardinalityRestrictedAspectHelper")
object CardinalityRestrictedAspectHelper {
  ...
  : String 
  = "CardinalityRestrictedAspects.json"

  implicit val decodeCardinalityRestrictedAspect: Decoder[CardinalityRestrictedAspect]
  = Decoder.instance[CardinalityRestrictedAspect] { c: HCursor =>
    
    import cats.syntax.either._
  
    for {
    	  uuid <- c.downField("uuid").as[taggedTypes.CardinalityRestrictedAspectUUID]
    	...
    	} yield CardinalityRestrictedAspect(
    	  uuid,
    	  tboxUUID,
    	  restrictedRangeUUID,
    	  name,
  ...
  implicit val encodeCardinalityRestrictedAspect: Encoder[CardinalityRestrictedAspect]
  = new Encoder[CardinalityRestrictedAspect] {
    override final def apply(x: CardinalityRestrictedAspect): Json 
    = Json.obj(
    	  ("uuid", taggedTypes.encodeCardinalityRestrictedAspectUUID(x.uuid)),
  ...
  @JSExport
  def toJSON(c: CardinalityRestrictedAspect)
  : String
  = encodeCardinalityRestrictedAspect(c).noSpaces
  ...
  = parse(c) match {
  	case scala.Right(json) =>
  	  decodeCardinalityRestrictedAspect(json.hcursor) match {
